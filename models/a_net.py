...
class AVNet(nn.Module):
    ...
    def encode(self, inputBatch):
        ...
        audioInputBatch = inputBatch[0]

        # audio conformer encode
        if audioInputBatch is not None:
            audioInputBatch = audioInputBatch.transpose(0, 1)#(B, T, inSize)
            audioBatch, _ = self.audioEncoder(audioInputBatch, audioInputBatch.size(1))# (B, T, encoder_dim=512), (B,)
        ...
        if (audioBatch is not None):
            # MLPs
            audioBatch = self.linear1(audioBatch)
            audioBatch = audioBatch.transpose(0,1).transpose(1,2)
            audioBatch = self.batchnorm(audioBatch)
            audioBatch = audioBatch.transpose(1,2).transpose(0,1)
            audioBatch = self.relu(audioBatch)
            audioBatch = self.linear2(audioBatch)
