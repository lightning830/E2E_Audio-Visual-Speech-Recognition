...
class VNet(nn.Module):
    ...
    def encode(self, inputBatch):
        ...
        if videoInputBatch is not None:
            ...
            videoBatch = videoBatch.transpose(0, 1)#(B,T,inSize)
            # conformer fornt-end
            for layer in self.videoEncoder:
                videoBatch = layer(videoBatch)
            videoBatch = videoBatch.transpose(0, 1)#(T,B,inSize)
        else:
            videoBatch = None

        # MLPs
        if videoBatch is not None:
            videoBatch = self.linear1(videoBatch)
            videoBatch = videoBatch.transpose(0,1).transpose(1,2)
            videoBatch = self.batchnorm(videoBatch)
            videoBatch = videoBatch.transpose(1,2).transpose(0,1)
            videoBatch = self.relu(videoBatch)
            videoBatch = self.linear2(videoBatch)
