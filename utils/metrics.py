...
def compute_cer(predictionBatch, targetBatch, predictionLenBatch, targetLenBatch):
    ...
    for n in range(len(preds)):
        pred = preds[n].numpy()[:-1].astype('int32')
...
def compute_cer_att(predictionBatch, targetBatch, targetLenBatch, eosIx):

    targetBatch = targetBatch.cpu()
    targetLenBatch = targetLenBatch.cpu()

    preds = list()
    pre_preds = predictionBatch[1:].T.numpy() #(B, T)

    # remove after eosIx
    for pred in pre_preds:
        pred_tmp = np.empty(0, dtype=int)
        for chara in pred:
            if chara == eosIx:
                break
            pred_tmp = np.append(pred_tmp, chara)
...
def compute_wer_att(predictionBatch, targetBatch, targetLenBatch, eosIx, spaceIx):
    ...
    # remove after eosIx
    for pred in pre_preds:
        pred_tmp = np.empty(0, dtype=np.int32)
        for chara in pred:
            if chara == eosIx:
                break
            pred_tmp = np.append(pred_tmp, chara)
