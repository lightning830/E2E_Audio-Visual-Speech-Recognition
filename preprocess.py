...
def load_model(load_path, model, device):
    pretrained_dict = torch.load(load_path, map_location=device)
    pretrained_dict = pretrained_dict['model_state_dict'] #こういうdictになってるから仕方ない一文
...
def main():
    ...
    vf = VisualFrontend()
    vf = load_model(args["TRAINED_FRONTEND_FILE"], vf, device)
    vf.to(device)
