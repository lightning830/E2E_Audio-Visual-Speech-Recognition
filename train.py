...
import logging
from models.a_net import AVNet
...
def main():
    ...
    matplotlib.use("Agg") # nonGPIでsavefigするとerror
    ...
    if args["PRETRAINED_MODEL_FILE"] is not None:
        ...
        model.load_state_dict(torch.load(args["CODE_DIRECTORY"] + args["PRETRAINED_MODEL_FILE"], map_location=device), strict=False)
        # model.to(device)
        print("Loading Done.\n")
    ...
    logging.basicConfig(format=formatter,level=logging.INFO,filename='checkpoints/train.log')
    ...
    for step in range(args["NUM_STEPS"]):
        ...
        validationLoss, validationCER, validationWER, validationCER_att, validationWER_att, predictionBatchs, predictionBatchs_att, targetBatchs =\
             evaluate(model, valLoader, loss_function, device, valParams)
        ...
        out = ("Step: %03d || Tr.Loss: %.6f Val.Loss: %.6f || Tr.CER: %.3f  Val.CER: %.3f attCER: %.3f || Tr.WER: %.3f  Val.WER: %.3f attWER: %.3f"
        ...
        if ((step%args["SAVE_FREQUENCY"] == 0) or (step == args["NUM_STEPS"]-1)) and (step != 0):
            ...
            savePath = args["CODE_DIRECTORY"] + "/audio_visual/checkpoints/models/train-step_{:04d}-wer_{:.3f}.pt".format(step, validationWER)
            ...
            plt.legend()
            plt.savefig(args["CODE_DIRECTORY"] + "/audio_visual/checkpoints/plots/train-step_{:04d}-loss.png".format(step))
            plt.close()
            ...
            plt.legend()
            plt.savefig(args["CODE_DIRECTORY"] + "/audio_visual/checkpoints/plots/train-step_{:04d}-wer.png".format(step))
            plt.close()
